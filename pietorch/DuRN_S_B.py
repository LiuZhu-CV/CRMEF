...
class dereain_net(nn.Module):
    def __init__(self, test_with_multigpus=False):
        super(dereain_net, self).__init__()
        self.main = cleaner(test_with_multigpus)
        self.task_l = inner_restoration(test_with_multigpus)
        self.task_m = inner_restoration(test_with_multigpus)
        self.task_h = inner_restoration(test_with_multigpus)
    def forward(self, x):
        x, out, res =self.main(x)
        outl =self.task_l(x,out,res)
        outm = self.task_m(x, out, res)
        outh = self.task_h(x, out, res)
        return outl, outm, outh
...
class inner_restoration(nn.Module):
    def __init__(self, test_with_multigpus=False):
        super(inner_restoration, self).__init__()
